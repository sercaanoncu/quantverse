...
| portfolio theory rules | risk parity | Portfolio Optimization | Risk parity balances risk contributions rather than capital weights. | actually_run_in_v2_with_public_data | Keep risk contribution and out-of-sample evidence visible. |
| portfolio theory rules | HRP | Portfolio Optimization; Machine Learning for Algorithmic Trading | Hierarchical risk parity uses correlation clustering to allocate risk more robustly. | actually_run_in_v2_with_public_data | Retain only while model-selection and history-integrity gates pass. |
...
| data/source/FX rules | FX normalization | market-data and portfolio accounting practice | Non-USD local returns must be converted for a USD portfolio. | implemented_with_public_data_provider_limits | Add independent vendor/calendar reconciliation before institutional promotion. |
| data/source/FX rules | local-currency vs USD returns | portfolio accounting practice | Local returns and base-currency returns answer different questions. | implemented_with_explicit_local_and_usd_outputs | Retain quote-direction, calendar and compounding checks. |
...
| data/source/FX rules | security identity and ticker reuse | Machine Learning for Algorithmic Trading; portfolio-data governance; official exchange and regulatory notices | Ticker symbols are time-varying routing labels; known reuse requires issuer/security identifiers and an effective listing boundary. | implemented_with_manual_primary_source_overrides | Expand dated security-master coverage and independent reference-data reconciliation. |
| data/source/FX rules | IPO and feature history sufficiency | Portfolio Optimization; Introduction to Statistical Learning; Machine Learning for Algorithmic Trading | A feature is eligible only when its stated lookback is fully observed before the scoring date. | implemented_conservative_standard_exclusion | Keep short-history securities visible as diagnostic-only until 252 valid returns exist. |
