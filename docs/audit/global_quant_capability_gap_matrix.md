...
| 10 | Does it ingest crypto top 100 by market cap? | `partial_metadata_only` | CoinGecko market-cap API rows and CoinGecko IDs are populated. | CoinGecko symbols are not a verified Yahoo/provider crosswalk; current rows are not investable until mapping evidence exists. | Add and review an exchange/vendor crypto price crosswalk keyed by CoinGecko ID. |
| 11 | Does it flag/exclude stablecoins where appropriate? | `implemented_conservative_gate` | Shared stablecoin/stable-value detection is enforced in universe, scoring and portfolio inputs. | The operational taxonomy needs periodic review and source-backed classification. | Add a maintained stable-value taxonomy source and regression fixtures. |
