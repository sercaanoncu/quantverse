...
REGION_BY_SLEEVE = {
    "global_equity_nasdaq": "North America",
    "global_equity_nyse": "North America",
    "global_equity_us": "North America",
    "global_equity_europe": "Europe",
    "global_equity_germany": "Europe",
...
EXTRA_OUTPUT_COLUMNS = ["source_url", "source_method"]
...
def _normalize_source_frame(df: pd.DataFrame, sleeve: str) -> pd.DataFrame:
    rows = []
    for source_order, row in df.reset_index(drop=True).iterrows():
        market_cap = pd.to_numeric(row.get("market_cap_usd", pd.NA), errors="coerce")
        row_sleeve = str(row.get("sleeve", "") or sleeve)
        row_region = str(row.get("region", "") or REGION_BY_SLEEVE.get(row_sleeve, ""))
        rows.append(
            {
                "ticker": row["ticker"],
                "name": row["name"],
                ...
                "benchmark_only": row.get("benchmark_only", False),
                "signal_only": row.get("signal_only", False),
                "include": row.get("include", True),
                "proxy_type": row.get("proxy_type", "direct_listing"),
                "source_method": row.get("source_method", "manual_review_required"),
                "notes": row["notes"],
                "_source_order": int(source_order),
            }
        )
...
def _rank_and_select_by_market_cap(
    ...
    for _, sleeve_df in universe.groupby("sleeve", sort=True):
        ...
        if not missing.empty:
            missing = (
                missing.sort_values("_source_order").head(int(top_n_per_sleeve)).copy()
            )
            missing["include"] = missing["include"].map(_to_bool)
            missing["notes"] = (
                missing["notes"].fillna("").astype(str)
                + " | Missing market cap/rank; selected only as sourced index/proxy constituent, not as top-by-market-cap evidence."
            )
            selected.append(missing)
    if not selected:
        return _empty_universe()
    output = pd.concat(selected, ignore_index=True)
    output = output.drop(columns=["_source_order"], errors="ignore")
    ordered = REQUIRED_UNIVERSE_COLUMNS + [
        column for column in EXTRA_OUTPUT_COLUMNS if column in output
    ]
...
def _empty_universe() -> pd.DataFrame:
    return pd.DataFrame(columns=REQUIRED_UNIVERSE_COLUMNS + EXTRA_OUTPUT_COLUMNS)
